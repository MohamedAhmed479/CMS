...
<?php 
                if(isset($_GET['category_id']) && is_numeric($_GET['category_id'])) {
                    ...
                    $query = "SELECT count(*) as total FROM posts WHERE category_id = $post_category_id";
                    ...
                    if($page > $number_of_pages) {
                        header("location: {$_SERVER['PHP_SELF']}?page=$number_of_pages&&category_id=$post_category_id");
                    }
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            <?php
                ...
                if(mysqli_num_rows($runQuery) == 1) {
                    ...
                    if(mysqli_num_rows($runQuery) >= 1) {
                        ...
                        $msg = "<div style='background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; padding: 10px; margin: 10px 0; border-radius: 5px;'>$category_title Has No Posts</div>";
                    }
                } else {
                    $msg = "<div style='background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; padding: 10px; margin: 10px 0; border-radius: 5px;'>This Category Not Found!</div>";
                }
            ?>

            <?php if(isset($all_posts)) : ?>
                <h1 class="page-header" style="margin-top: 20px; font-size: 30px; text-align: center;">
                    All posts are for the <?php echo $category_title ?> category
                </h1>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item <?php if($page == 1) echo "disabled" ?>"><a class="page-link" href="category.php?page=<?php  echo $page - 1 ?>&&category_id=<?php echo $post_category_id ?>"> Previos </a></li>
                    <li class="page-item"><a class="page-link" href="#">Page <?php echo $page ?> of <?php echo $number_of_pages ?></a></li>
                    <li class="page-item <?php if($page == $number_of_pages) echo "disabled" ?>"><a class="page-link" href="category.php?page=<?php echo $page + 1 ?>&&category_id=<?php echo $post_category_id ?>"> Next </a></li>
                </ul>
            </nav>
