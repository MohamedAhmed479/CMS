...
if(isset($_POST['submit_Edit_user']) && is_numeric($_POST['user_id'])) {
    ...
    $query = "SELECT email, image, password FROM users WHERE id = $user_id";
    $runQuery = mysqli_query($conn, $query);

    if(mysqli_num_rows($runQuery) == 1) {
        $user_information = mysqli_fetch_assoc($runQuery);
        $old_image = $user_information['image'];
        $old_password = $user_information['password'];

        $query = "SELECT email FROM users WHERE email = '$email' && id != $user_id";
        $runQuery = mysqli_query($conn, $query);
        if(mysqli_num_rows($runQuery) == 1) {
            $errors[] = "This email is alredy exist.";
        }
        $query = "SELECT username FROM users WHERE username = '$username' && id != $user_id";
        $runQuery = mysqli_query($conn, $query);
        if(mysqli_num_rows($runQuery) == 1) {
    ...
    if(empty($errors)) {
        ...
        // handle password
        if($no_new_password) {
            $new_password_hashed = $old_password;
        } else {
        ...
        $query = "UPDATE users SET first_name = '$first_name', last_name = '$last_name', username = '$username',
                    email = '$email' ,image = '$new_image_name', password = '$new_password_hashed', role = '$user_role'
                WHERE id = $user_id";
        $runQuery = mysqli_query($conn, $query);
        if($runQuery) {
            ...
            // save error msg
            $_SESSION['errors'] = ['Error While Edit'];
